...
export const filterListRequest = (value) => {
    return (dispatch) => {
        return axios.get(Config.API_URL + '/category/filter-list', {params: {value: value}}).then(res => {
            dispatch(filterList(res.data.data));
        })
    }
}

export const filterList = (categories) => {
    return {
        type: Types.FILTERLIST_CATEGORIES,
        categories
    }
}
